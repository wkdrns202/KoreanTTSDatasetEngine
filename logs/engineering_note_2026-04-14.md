...
# [APPEND #10] 2026-04-18 — Pipeline Rerun + D9 검증 + 적응형 Calibration 연구 방향

## 1. 오늘 실행한 작업 요약

### 1-1. Stage 1+2 align_and_split 전체 재실행 (00:51~00:54)

`logs/last_run_params.json` 기준:

```json
{
  "timestamp": "2026-04-18T00:52:50",
  "whisper_model": "medium",
  "audio_pad_ms": 320,         ← 변경 (직전 50)
  "fade_ms": 3,                ← 변경 (직전 10)
  "search_window": 25,
  "skip_penalty": 0.01,
  "match_threshold": 0.5,
...
**산출물**: `datasets/rerun_excl_part2/`
- `wavs/` **5,443 WAV** (script별 951/1308/873/998/991/322), `script.txt` 5,443 entries
- 4,033 = 이번 run에서 (re-)matched. 5,443 = 기존 checkpoint 보존된 매치 + 이번 rerun 포함 누적치 (`--resume` 기본 동작)
- Stage 2 R6 envelope: 400ms pre-attack + 730ms tail, 3ms raised-cosine fade, -1.0dB peak normalize

`feedback_no_overwrite` 원칙 준수 — `datasets/wavs/` 캐노니컬은 손대지 않음.

### 1-2. evaluate_dataset Stage 3+4 (06:20)

- `logs/eval_checkpoint.json`, `logs/evaluation_report.json` 갱신
- 이번엔 **extended scoring 활성화** — `similarity_unprompted`, `avg_logprob` 컬럼 생성됨 (직전 4196개 평가는 prompted-only였음)

### 1-3. selective_composer Stage 4.5 (06:22~06:23)

`logs/composition_report.json` 핵심 결과:

```
total_segments      5,443
ACCEPT              4,793  (88.06%)
PENDING                 0
REJECT                650
total_audio          8.42 h   (ACCEPT 7.16 h, REJECT 1.25 h)
human_review_count    281

D1 S_unprompted    mean 0.92  std 0.18  ← 신규 활성화
D2 S_gap           mean 0.97  std 0.07
D3 S_snr           mean 1.00            (예상대로 — 데이터 quality 자체 양호)
D4 S_duration      mean 0.81  std 0.21
D6 S_confidence    mean 0.76  std 0.12  ← 신규 활성화 (직전 default 0.5)
D7 S_boundary      mean 1.00            (강제 envelope 주입 — 정상)
D8 S_continuity    mean 1.00            (Option B trim 효과)
D9 S_decay         mean 0.87  std 0.28  ← 핵심 개선 (직전 0.18)
```

**`d9_reject_review/` 폴더 410개 wav 별도 분류** — D9 score < 0.3 청취 검증 대상.

## 2. D9 S_decay의 데이터 품질 변별력 — 결정적 검증

직전 `datasets/wavs/` (옛 pipeline 산출물, 2026-02-11 기준)과 신 `datasets/rerun_excl_part2/`의 D9 분포 비교:

| Dataset | 평균 D9 | 해석 |
|---|---|---|
| 옛 (zero-fill POSTSPEECH, fade 5ms) | **0.18** | span 짧음 → cliff signature |
| 신 (POSTSPEECH 700ms natural + 30ms fade) | **0.87** | 자연 decay 보존 |

**의미**:

- D9는 dead dimension이 아님 — pipeline 변화에 민감하게 반응
- 신 pipeline의 `POSTSPEECH_PAD = 700ms (natural-preserve)` + `FADE_OUT_MS = 30` 조합이 자연 decay tail을 유지함을 정량 확인
- D9 알고리즘 자체는 **본 dataset에서 의도대로 작동**

이로써 APPEND #9에서 설계한 D9의 detection 역할이 실제 pipeline 변경 검증 도구로 확인됨.

## 3. 9차원 점수의 현재 상태 정리

| Dim | 평균 | 상태 | 비고 |
|---|---|---|---|
| D1 unprompted | 0.92 | 활성 | extended eval 필요, 이번 run으로 활성화 |
| D2 gap | 0.97 | 활성 | unprompted-prompted 차이 |
| D3 snr | 1.00 | constant | 데이터 quality 우수 (의도된 결과) |
| D4 duration | 0.81 | 활성 | session AST z-test |
| D5 stability | — | optional | PENDING 풀 전용, 이번 run엔 PENDING=0 |
| D6 confidence | 0.76 | 활성 | extended eval 필요, 이번 run으로 활성화 |
| D7 boundary | 1.00 | constant | 강제 envelope 주입 정상 (의도) |
| D8 continuity | 1.00 | constant | Option B bimodality trim 효과 (의도) |
| D9 decay | 0.87 | 활성 | 본 run에서 변별력 검증됨 |

활성 dimension: D1, D2, D4, D6, D9 (+ optional D5)
Constant dimension: D3, D7, D8 — 모두 "이렇게 나오는 게 의도된 결과"임이 확인됨

## 4. 시각화 산출물 (어제 작업 기준)

- `docs/dimension_histograms.png` — verdict별 차원 분포
- `docs/dimension_correlation.png` — Spearman 상관 히트맵
- `docs/dimension_radar.png` — verdict 평균 프로파일
- `docs/d9_diagnostic.png` — HEAD/TAIL 분리 진단

(주: 시각화는 **옛 데이터**(0.18 mean) 기준이라 신 데이터로 재생성하면 분포 형태 크게 달라질 것 — TODO)

## 5. 연구 방향 분리 — 적응형 Calibration

오늘 D9 알고리즘 리뷰 중 발견된 본질적 한계: **span/gradient 임계값이 화자/녹음환경 무관한 절대값**. 

화자별 끝음 처리 습관(단호 vs 여운)과 환경별 reverb tail 길이가 자연 span에 반영되는데, 현재 임계값은 이를 흡수하지 못함. 임계값 자체도 empirical 측정이 아닌 gradient 임계값에서 수학적으로 역산된 정합성 값.

**Empirical Bayes / James-Stein shrinkage** 기반 세션 적응형 calibration이 통계학적으로 정당한 해법으로 식별됨. 표본 contamination 문제, 환경 효과와 화자 효과의 분리, ICC 사전 검증, GMM-EM 업그레이드 옵션 등 종합적 검토가 필요한 별도 연구 라인.

→ **`docs/research/d9_session_adaptive_calibration.md` 로 분리 정리**.

선행연구 매핑:
- James & Stein (1961), Efron & Morris (1973, 1975) — Stein paradox / EB shrinkage
- Gelman BDA3 §5, Rubin (1981) 8-schools — hierarchical Bayesian
- Huber (1964), Rousseeuw (1984) — robust statistics
- Reynolds & Rose (1995), McLachlan & Peel (2000) — GMM/EM
- Sports analytics, CUPED A/B testing — 산학 hierarchical pooling 사례

본 연구의 적용 영역(음성 dataset cleanser의 quality gate에 hierarchical adaptive calibration)은 음성 QA 도메인에서 상대적으로 새 지점.

## 6. Cross-cutting 후속 작업

- **D9 시각화 재생성** — 신 데이터(0.87 mean) 기준 dimension_*.png 갱신
- **`d9_reject_review/` 410개 청취** — 진짜 truncation vs false positive 비율 확인
- **D4 적응형 검토** — D9와 같은 mean/std 한계가 있음. 연구 노트에서 D4도 같은 패턴으로 개선 가능성 명시
- **MEMORY.md 갱신** — Final Dataset State 섹션이 2026-02-11 기준 4196개로 박혀있음. 신 pipeline 산출 5,443개 / `datasets/rerun_excl_part2/` 경로 추가 필요
- **Pipeline params 메모리 갱신** — `AUDIO_PAD_MS = 50 → 320`, `FADE_MS = 10 → 3` 변경 반영

## 7. 미해결 의문

- AUDIO_PAD_MS 50 → 320 변경 근거? (오늘 run 기록만 있고 실험 결과/사유 미기록 — 다음 세션에서 보충 필요)
- FADE_MS 10 → 3 변경은 어디서 결정됐는지?

---

# [APPEND #9] 2026-04-18 — Rerun 결과 분석 + Multi-Model/Multi-Parameter 설계

## 1. Rerun Pipeline 실행 결과

### 1.1 배경

Part1(3,805 WAVs, 7.49h)에서 끝음절 truncation 발생 → Part2 ACCEPT 1,410라인을 제외하고 raw audio부터 전체 재처리.
D9 S_decay 포함 최신 9차원 체계로 실행.

### 1.2 Pipeline (Stage 1+2) 결과

| Script | Target | Matched | Rate |
|--------|--------|---------|------|
| S1 | 342 | 309 | 90.4% |
| S2 | 1,416 | 1,308 | 92.4% |
| S3 | 878 | 873 | 99.4% |
...
| S5 | 572 | 544 | 95.1% |
| S6 | 9 | 1 | 11.1% |
| **Total** | **4,222** | **4,033** | **95.5%** |

Part2 merge 후 통합: **5,443 WAVs, 10.71h**

### 1.3 Stage 4.5 결과 (evaluate 포함)
...
| REJECT | 650 |
| ACCEPT 오디오 | 9.70h |
| Calibrated tau_accept | 0.6441 |
| Calibrated tau_reject | 0.8325 (역전 → PENDING 소멸) |
...
| HUMAN_REVIEW_NEEDED | 220 |
| formal_ending_truncation_risk | 35 |

## 2. D9 S_decay 과민 반응 분석

### 2.1 증상

409건이 S_decay < 0.5로 reject. 그 중 295건은 **D9만 문제** — 나머지 8개 차원은 정상.

### 2.2 원인

1. **Span score**: body_end(-40dB)와 last_signal(-68dB) 거리가 ≤ 10ms(2 windows) → span_score=0.0
2. **Logic Pro 편집**: 사용자가 DAW에서 tail을 약간 cut한 경우, 자연 감쇠 구간이 짧아져 span이 줄어듦
3. 이 정도의 cut은 **청취상 문제없음** — 사용자 확인 완료

### 2.3 이웃 문장 혼입 미감지 (Script_1_0066.wav)

```
본문 speech: 500-4590ms
Gap: 610ms (4590-5200ms)
이웃 speech: 5200-5360ms (160ms, -25dB)
```

- D8 S_continuity = 1.0 (gap 610ms < threshold 730ms → 통과)
- D9 S_decay = 0.0 (이웃 문장이 voiced 끝까지 이어져 body_end ≈ last_signal)
- **D9가 이웃 문장 혼입의 부수 증상을 잡은 것** (본래 D8의 역할)

### 2.4 향후 조치

- D9 gradient/span threshold 완화 검토 (Logic 편집 데이터에 과민)
- D8 CONTINUITY_GAP_MS 500-600ms 하향 검토 (610ms gap 미감지 문제)
- 단, 전체 영향도 분석 후 결정
...
`tau_accept(0.6441) < tau_reject(0.8325)` → PENDING 구간 소멸.

### 3.2 원인

- known_good(unprompted >= 0.95)의 composite 5th percentile → 0.6441
- known_bad(unprompted < 0.60)의 composite 95th percentile → 0.8325
- bad 샘플의 composite가 높음: unprompted만 낮고 나머지 8차원은 OK → composite 올라감

### 3.3 향후 조치

역전 방지 guard 추가 필요: `tau_accept = max(tau_accept, tau_reject + margin)`

## 4. Multi-Parameter Robustness 검증 — 설계 검토

### 4.1 아이디어

SAM처럼 threshold를 여러 조합으로 변형하여 측정, 모든 기준에서 살아남는 데이터만 ACCEPT.

### 4.2 단순 sweep의 한계 (사용자 피드백)

**각 dimension의 물리적 의미를 무시한 naive sweep은 무의미:**

1. **D8 (continuity)**: 상류에서 730ms 기준으로 이미 음성을 절단. 800/900ms sweep은 검증 불가 — 730ms 초과 gap이 존재할 수 없음
2. **D9 (decay)**: span 10/50에서 reject → 20/50에서도 당연히 reject. 10/50 통과 → 20/50 미통과는 화자의 자연 발화 습관일 수 있음. 민감도 증가 ≠ 신뢰도 증가
3. **D4 (SNR)**: 40dB 통과 → 30dB도 당연히 통과. 50dB 기준은 peak 음성을 우대하는 오류
4. **D5 (duration)**: 통계적 유의성 범위 내 변동을 허용하는 게 취지. sweep은 이 취지와 충돌

### 4.3 유효한 방향: Multi-Model Consensus

단일 dimension 내 threshold sweep보다, **다른 ASR 모델로 독립 전사 → 교차 검증**이 더 의미 있는 다양성 제공.

## 5. Multi-Model Alignment 설계 (Stage 1+2 진화)

### 5.1 현재 구조

```
Raw Audio → [Whisper medium] → segments → alignment → split → WAV
```

### 5.2 제안 구조

```
Raw Audio → [Whisper medium]   → segments_A ─┐
          → [Qwen3-ASR-1.7B]  → segments_B ─┼→ Consensus Selector → best split
          → [VibeVoice-ASR]   → segments_C ─┘
```

### 5.3 후보 ASR 모델

| 모델 | 출처 | 한국어 | Timestamp | VRAM | 특장점 |
|------|------|--------|-----------|------|--------|
| Whisper medium | OpenAI | ✅ | word-level | ~5GB | 현행 기준선 |
| **Qwen3-ASR-1.7B** | Alibaba | ✅ 52개 언어 | word/char (ForcedAligner) | ~8GB | Whisper-large-v3 상회 벤치마크 |
| **VibeVoice-ASR** | Microsoft | ✅ 50+ 언어 | word-level + 화자분리 | TBD | 60분 단일 패스, MIT 라이선스 |

### 5.4 아키텍처 다양성

- Whisper: encoder-decoder transformer
- Qwen3-ASR: LLM 기반 (Qwen backbone)
- VibeVoice: continuous speech tokenizer + ultra-low frame rate

**3개 모델의 아키텍처가 모두 다름** → 동일 오류 패턴을 공유할 가능성 낮음 → consensus의 의미가 있음.

### 5.5 Consensus Selector 설계 원칙

```python
for each script_line:
    split_A = whisper의 (start_ms, end_ms)
    split_B = qwen3의 (start_ms, end_ms)
    split_C = vibevoice의 (start_ms, end_ms)
    
    # Boundary agreement: 3개 모델의 start/end가 ±50ms 이내 → high confidence
    # Text agreement: 전사 결과 유사도 투표
    # Duration consistency: AST 정상 범위 여부
    # Fallback: agreement 없으면 가장 보수적인 split 채택
```

### 5.6 실행 전략

- 순차 실행: Model A 전사 → GPU 해제 → Model B → GPU 해제 → Model C
- transcription cache: 각 모델 결과를 JSON으로 저장, alignment은 일괄 수행
- VRAM 보강 예정 (사용자 확인), 부족 시 cloud GPU 활용

### 5.7 단계적 로드맵

| Phase | 내용 | 비고 |
|-------|------|------|
| Phase 1 | D9 threshold 조정 | 295건 오탐 복구 (즉시) |
| Phase 2 | Qwen3-ASR PoC | 소규모 전사 테스트 + Whisper 결과 비교 |
| Phase 3 | VibeVoice PoC | 소규모 전사 테스트 |
| Phase 4 | TranscriptionResult 표준 포맷 | model-agnostic 인터페이스 설계 |
| Phase 5 | Consensus Selector 구현 | 3-model 통합 alignment |
