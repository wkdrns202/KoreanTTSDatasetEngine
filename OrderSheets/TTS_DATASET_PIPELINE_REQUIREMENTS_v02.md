...
| **R6** | **Audio Envelope Compliance (Pre-attack & Tail Silence)** | Every sliced WAV MUST have: (a) **Pre-attack silence ≥ 50ms (0.05s)** — guaranteed quiet region before the first voiced sample, and (b) **Tail silence ≥ 300ms (0.3s)** — guaranteed quiet region after the last voiced sample. Silence is defined as RMS < -40dB. If the original slice lacks sufficient silence, the pipeline MUST **pad with digital silence** (zero-samples) to meet the minimum. Measured per-segment; ≥ 95% of all segments must comply. | -- | --% |
...
| `PREATTACK_SILENCE_MS` | `200` | **[R6]** Minimum silence before voice onset. Pipeline MUST pad with zero-samples if natural silence is insufficient. |
| `TAIL_SILENCE_MS` | `300` | **[R6]** Minimum silence after voice offset. Pipeline MUST pad with zero-samples if natural silence is insufficient. |
| `SILENCE_THRESHOLD_DB` | `-40` | RMS threshold (dB) for silence detection — used in R2 boundary checks AND R6 voice onset/offset detection |
...
For each segment WAV in datasets/wavs/:
    ...
    4. === AUDIO ENVELOPE ENFORCEMENT (R6 — CRITICAL) ===
       a. Measure Pre-attack region: find the first sample where RMS (in 10ms window) exceeds -40dB.
          - If silence before first voice < 50ms → prepend zero-samples to reach exactly 50ms.
          - If silence before first voice > 50ms → trim to exactly 50ms.
       b. Measure Tail region: find the last sample where RMS (in 10ms window) exceeds -40dB.
          - If silence after last voice < 300ms → append zero-samples to reach exactly 300ms.
          - If silence after last voice > 300ms → trim to exactly 300ms.
       c. This ensures EVERY WAV has a consistent audio envelope:
          [50ms silence] + [voiced speech] + [300ms silence]
    5. Normalize peak amplitude to -1dB (normalize ONLY the voiced region, not the padded silence)
    6. Re-export as WAV (48KHz, 24-bit, mono — preserve source quality)
```

> ⚠️ **R6 ENFORCEMENT IS NON-NEGOTIABLE.** The pre-attack (50ms) and tail silence (300ms) values are TTS training requirements. Inconsistent silence regions cause model artifacts during synthesis. The pipeline must guarantee these values algorithmically — never rely on the source audio having natural silence.
...
For each segment WAV:
    ...
    5. Audio envelope check (R6):
       a. Detect voice onset: first sample where RMS (10ms sliding window) >= -40dB
       b. Detect voice offset: last sample where RMS (10ms sliding window) >= -40dB
       c. pre_attack_ms = (voice_onset_sample / sample_rate) * 1000
       d. tail_silence_ms = ((total_samples - voice_offset_sample) / sample_rate) * 1000
       e. R6_pass = (pre_attack_ms >= 48ms) AND (tail_silence_ms >= 295ms)
          [Allow 2ms/5ms tolerance for sample-boundary rounding]
    6. Verdict:
       PASS = (similarity >= 0.95) AND (R2 boundary checks pass) AND (R6 envelope pass)
```
...
```json
{
  ...
  "r6_audio_envelope_pass_rate": 0.0,
  "r6_preattack_stats": {
    "min_ms": 0.0,
    "max_ms": 0.0,
    "mean_ms": 0.0,
  ...
  "failed_segment_details": [
    {
      ...
      "r6_preattack_ms": 12.0,
      "r6_tail_silence_ms": 150.0,
      "r6_envelope_pass": false,
      "failure_reason": "similarity below threshold; pre-attack too short (12ms < 50ms); tail too short (150ms < 300ms)",
      "failure_type": "Type A, Type F"
    }
  ]
}
```
...
3. Generate `datasets/validation_results.csv` with columns: `filename | ground_truth | whisper_text | similarity | boundary_pass | preattack_ms | tail_silence_ms | envelope_pass | verdict`
...
1. CATEGORIZE all failed segments:
   ...
   - Type F: Audio envelope violation (pre-attack < 50ms OR tail silence < 300ms)
...
3. RESEARCH & APPLY targeted fixes:
   ...
   For Type F (Audio Envelope Violation — Pre-attack / Tail Silence):
   -> DIAGNOSIS: Check if STAGE 2 envelope enforcement step is executing correctly
   -> FIX 1: Verify voice onset/offset detection uses correct RMS window size (10ms)
   -> FIX 2: Verify silence threshold is -40dB (not accidentally using linear scale)
   -> FIX 3: Check sample-rate math — 50ms at 48kHz = 2400 samples, 300ms = 14400 samples
   -> FIX 4: If voiced region is being detected too early (capturing noise as voice):
      - Increase RMS threshold (e.g., -40dB -> -35dB for onset detection)
      - Use energy + zero-crossing-rate combined detection
      - Apply high-pass filter (>80Hz) before onset detection to ignore DC offset / low-freq rumble
   -> FIX 5: If silence is being trimmed AFTER padding was added:
      - Ensure padding is added as the FINAL step, after all trimming and normalization
      - Reorder STAGE 2 steps: trim first → normalize → pad last → export
   -> FIX 6: If fade-in/fade-out is eating into the silence region:
      - Apply fades to the VOICED region only, not to the padded silence
   -> VALIDATION: After fix, re-measure all segments and confirm:
      - pre_attack_ms >= 48ms (with 2ms tolerance)
      - tail_silence_ms >= 295ms (with 5ms tolerance)
...
- R6 (Audio Envelope Pass): ___%
  - Pre-attack min/mean: ___ms / ___ms
  - Tail silence min/mean: ___ms / ___ms
...
- Type F (Envelope Violation): ___ segments
  - Pre-attack failures: ___
  - Tail silence failures: ___
...
REPEAT {
    ...
    3. Check evaluation_report.json:
       - If R1 >= 95% AND R2 >= 95% AND R6 >= 95% AND R3 >= 95%:
...
Per-Segment Score:
  ...
  # R6 Audio Envelope Check
  voice_onset = first sample index where RMS(10ms_window) >= -40dB
  voice_offset = last sample index where RMS(10ms_window) >= -40dB
  pre_attack_ms = (voice_onset / sample_rate) * 1000
  tail_silence_ms = ((total_samples - voice_offset) / sample_rate) * 1000
  envelope_pass = (pre_attack_ms >= 48ms) AND (tail_silence_ms >= 295ms)
  # [2ms/5ms tolerance for sample-boundary rounding at 48kHz]

  segment_pass = (similarity >= 0.95) AND boundary_clean AND envelope_pass

Overall Score:
  R1_score = (count(similarity >= 0.95) / total_segments) x 100
  R2_score = (count(boundary_clean == True) / total_segments) x 100
  R6_score = (count(envelope_pass == True) / total_segments) x 100
  R3_score = (count(segment_pass == True) / total_segments) x 100
  # R3 is the strictest — requires R1 AND R2 AND R6 all passing per segment.

  R1, R2, R6 must each be >= 95%.
  R3 (combined) must be >= 95%.
```
...
> **DO NOT** trim silence AFTER padding — padding must be the absolute LAST step before export.
...
> **DO** verify every WAV has exactly [50ms pre-attack] + [speech] + [300ms tail] envelope structure.
> The `align_and_split.py` file is the most critical code - iterate on it relentlessly.
> Korean text normalization is essential - handle jamo decomposition, spacing variants, and particles carefully.
> R6 (Audio Envelope) is a TTS training hard-requirement — inconsistent silence causes synthesis artifacts.
...
R3 (Combined Match Rate)     >= 95%   -- R1 AND R2 AND R6 all pass per segment
...
R6 (Audio Envelope)          >= 95%   -- Pre-attack >= 50ms AND Tail silence >= 300ms per WAV

-> ALL SIX must be satisfied. No exceptions.
```
